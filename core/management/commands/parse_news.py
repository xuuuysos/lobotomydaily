...
class Command(BaseCommand):
    """
    Management command class to run news parsers.
    """
    ...
    def handle(self, *args, **options):
        ...
        if options['date']:
            try:
                ...
                start_point = timezone.make_aware(
    ...
    def extract_text_from_url(self, url):
        """
        Parse details of an article to extract its paragraphs.
        """
        ...
        body_div = soup.find(
            'div',
            class_=re.compile(
                r'topic-body__content|article-text|b-text|content__text|article__body|article__text'
            )
        )
        if not body_div:
    ...
    def get_lenta_links(self, date_obj, ru_months):
        """
        Fetch and parse index page for Lenta.ru.
        """
        ...
        for a in soup.find_all('a', href=True):
            ...
            if href.startswith(date_pattern):
                ...
                raw_title = (
                    title_node.get_text(separator=' ')
                    if title_node else a.get_text(separator=' ')
                )
                title = re.sub(r'\s+', ' ', raw_title).strip()
                title = re.sub(r'\d{2}:\d{2}.*$', '', title).strip()

                pub_time = timezone.make_aware(
                    datetime.datetime(date_obj.year, date_obj.month, date_obj.day, 8, 0)
                )
                time_node = a.find('time') or a.find(class_=re.compile(r'date'))
                if time_node:
                    t_str = time_node.get_text(strip=True)
                    m_full = re.search(
                        r'(\d{2}):(\d{2}),\s*(\d{1,2})\s+([а-яА-Я]+)\s+(\d{4})',
                        t_str
                    )
                    if m_full:
                        ...
                        pub_time = timezone.make_aware(
                            datetime.datetime(int(y), mon_num, int(d), int(h), int(mn))
                        )
                    else:
    ...
    def get_fontanka_links(self, date_obj):
        """
        Fetch and parse index page for Fontanka.ru.
        """
        url = (
            f"https://www.fontanka.ru/{date_obj.year}/"
            f"{date_obj.month:02d}/{date_obj.day:02d}/all.html"
        )
        html = self.fetch_html(url)
        soup = BeautifulSoup(html, 'lxml')
        links = []
        ...
        for a in soup.find_all('a', href=True):
            ...
            if m_url and href.endswith("/"):
                ...
                if time_node:
                    ...
                    if m:
                        pub_time = timezone.make_aware(
                            datetime.datetime(y, mon, d, int(m.group(1)), int(m.group(2)))
                        )
    ...
    def get_ria_links(self, date_obj):
        """
        Fetch and parse index page for RIA.ru.
        """
        url = (
            f"https://ria.ru/{date_obj.year}"
        ...
        for a in soup.find_all('a', href=True):
            ...
            if href.startswith(date_pattern) and href.endswith(".html"):
                ...
                pub_time = timezone.make_aware(
                    datetime.datetime(date_obj.year, date_obj.month, date_obj.day, 12, 0)
                )
                if title and len(title) > 20:
                    links.append({
                        'url': href,
